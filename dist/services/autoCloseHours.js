...
export const autoCloseOpenHoursForUser = async (userId, companyId) => {
    ...
    for (const hour of openHours) {
        ...
        const isOldOpenHour = entryYear !== nowYear ||
            entryMonth !== nowMonth ||
            entryDay !== nowDay;
        if (!isOldOpenHour)
            continue;
        const autoExit = new Date(Date.UTC(entryYear, entryMonth, entryDay, 20, 0, 0));
        const totalMinutes = Math.max(0, Math.floor((autoExit.getTime() - entry.getTime()) / 60000));
